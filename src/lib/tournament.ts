...
function seededBracket(players: Participant[]) {
  const size = 2 ** Math.ceil(Math.log2(Math.max(2, players.length)));
  let seedOrder = [1, 2];
  while (seedOrder.length < size) {
    const bracketSize = seedOrder.length * 2;
    seedOrder = seedOrder.flatMap((seed) => [seed, bracketSize + 1 - seed]);
  }
  return seedOrder.map((seed) => players[seed - 1]?.id ?? null);
}
...
export function createTournament(name: string, type: TournamentType, names: string[]): Tournament { const participants = names.map((name) => ({ id: id(), name: name.trim() })).filter((p) => p.name); let rounds: Match[][] = []; if (type === "swiss") rounds = [swissRound(1, participants, calculateStandings(participants, []), [])]; if (type === "roundRobin") rounds = roundRobin(participants); if (type === "elimination") rounds = [eliminationRound(1, seededBracket(participants))]; return { id: id(), name, type, status: "active", participants, rounds, standings: calculateStandings(participants, rounds), createdAt: new Date().toISOString() }; }
